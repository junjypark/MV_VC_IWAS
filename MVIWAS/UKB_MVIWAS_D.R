...
for (gene in 1:nrow(hg)){ # nrow(hg)
  tryCatch({
    ...
    pvecs_gene <- rep(NA, ncol(coef$D))
    pvecs2_gene <- rep(NA, ncol(coef$D))
    pvecs3_gene <- rep(NA, ncol(coef$D))
    pvecs4_gene <- rep(NA, ncol(coef$D))
...
saveRDS(pvalues, paste0("chr",chr,"_UKBresults_D.rds"))
