...
for (gene in 1:nrow(hg)){ # nrow(hg)
  tryCatch({
    ...
    pvecs_gene <- rep(NA, ncol(coef$F))
    pvecs2_gene <- rep(NA, ncol(coef$F))
    pvecs3_gene <- rep(NA, ncol(coef$F))
    pvecs4_gene <- rep(NA, ncol(coef$F))
...
saveRDS(pvalues, paste0("chr",chr,"_UKBresults_F.rds"))